 ...
 * remote (ngrok) visitor forced to GUEST_GUILD_ID, or a no-login /dashboard-link
 * guest locked to the server in their link. Null when the user is free to switch.
 */
export async function lockedGuildId(): Promise<string | null> {
  // An explicit per-server /dashboard link wins over the static GUEST_GUILD_ID
  // default, so each server's link opens that server (not always GUEST_GUILD_ID).
  const store = await cookies();
  const fromLink = linkGuestGuildId(store.get(GUEST_LINK_COOKIE)?.value);
