...
grep -q 'CREATE TABLE IF NOT EXISTS public."SoundUploadRecovery"' "$durable_recovery_fixes_schema"
grep -q 'CREATE OR REPLACE FUNCTION public.prepare_sound_upload_recovery' "$durable_recovery_fixes_schema"
grep -q 'CREATE OR REPLACE FUNCTION public.mark_sound_upload_recovery_pending' "$durable_recovery_fixes_schema"
grep -q 'CREATE OR REPLACE FUNCTION public.complete_sound_upload_recovery' "$durable_recovery_fixes_schema"
grep -q '"leaseToken" uuid' "$durable_recovery_fixes_schema"
grep -q '"leaseExpiresAt" timestamp with time zone' "$durable_recovery_fixes_schema"
grep -q 'CREATE OR REPLACE FUNCTION public.heartbeat_sound_upload_recovery' "$durable_recovery_fixes_schema"
grep -q 'CREATE OR REPLACE FUNCTION public.claim_sound_upload_recovery' "$durable_recovery_fixes_schema"
grep -q 'CREATE OR REPLACE FUNCTION public.defer_sound_upload_recovery' "$durable_recovery_fixes_schema"
grep -q 'p_source_absent boolean' "$durable_recovery_fixes_schema"
grep -q 'p_playable_absent boolean' "$durable_recovery_fixes_schema"
grep -q 'p_outcome text' "$durable_recovery_fixes_schema"
grep -q 'p_stage_id uuid' "$durable_recovery_fixes_schema"
grep -q 'sourceMimeType' "$durable_recovery_fixes_schema"
grep -q 'p_previous_playable_path text' "$durable_recovery_fixes_schema"
! grep -q 'DROP FUNCTION IF EXISTS public.prepare_sound_trim_mutation(uuid, uuid, bigint, uuid, text)' "$durable_recovery_fixes_schema"
grep -q 'RETURN true;' "$durable_recovery_fixes_schema"
