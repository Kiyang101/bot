...
export async function middleware(req: NextRequest) {
  ...
  const roleResult = authUser
    ...
    : null;
  if (roleResult?.error)
    throw new Error(`read DashboardUser: ${roleResult.error.message}`);
  const realSession =
    devBypassUser() ??
    sessionUserFromSupabaseUser(
      authUser,
      roleFromDatabase(roleResult?.data?.role),
    );
  ...
  if (guildParam) {
    ...
    response.cookies.set(GUILD_COOKIE, guildParam, {
      path: "/",
      sameSite: "lax",
      maxAge: YEAR,
    });
  ...
  if (!isRemoteAdminConfig && !canAccess(user.role, pathname)) {
    return redirectWithSession(
      new URL(homePathFor(user.role), origin),
      supabaseResponse,
    );
  }
