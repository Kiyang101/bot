 ...
 * For clients whose media URLs require Proof of Origin, pass the value in
 * yt-dlp's CLIENT.CONTEXT+TOKEN form (usually generated by a PO-token plugin):
...
function authFlags(): Record<string, unknown> {
  ...
  // android is the anonymous client for public videos. With cookies, avoid
  // yt-dlp's logged-in tv_downgraded client:
  // YouTube currently returns "The page needs to be reloaded" for that client.
  // The explicit default + embedded pair is the cookie-compatible workaround.
  // An explicit client always wins.
  const poToken = process.env.YTDLP_PO_TOKEN?.trim();
  const client = process.env.YTDLP_PLAYER_CLIENT?.trim()
    || (poToken ? 'mweb' : (hasCookies ? 'default,web_embedded' : 'android'));
  const extractorArgs: string[] = [];
  if (client) extractorArgs.push(`player_client=${client}`);
  if (poToken) extractorArgs.push(`po_token=${poToken}`);
  if (extractorArgs.length) flags.extractorArgs = `youtube:${extractorArgs.join(';')}`;
  return flags;
}
...
/** Return the YouTube playlist id, including the dynamic id used by Mix/Radio. */
...
/**
 * YouTube Mix/Radio playlists are dynamic and have no finite playlist count.
 * Keep their import bounded so a single `/play` request cannot grow forever.
 */
function isMixPlaylist(input: string): boolean {
  return playlistId(input)?.startsWith('RD') ?? false;
}

function maxMixTracks(): number {
  const configured = Number.parseInt(process.env.YTDLP_MIX_MAX_TRACKS ?? '', 10);
 ...
 *   - playlist URL → every entry (Mix/Radio URLs are capped by maxMixTracks)
...
export async function resolve(
  ...
  if (playlistId(input)) {
    const mix = isMixPlaylist(input);
    const info = (await ytdlp(input, {
      ...JSON_FLAGS,
      ...authFlags(),
      flatPlaylist: true,
      yesPlaylist: true,
      ...(mix ? { playlistEnd: maxMixTracks() } : {}),
    })) as unknown as RawInfo;
    const entries = info.entries ?? [];
    return {
      tracks: entries.map((e) => toTrack(e, requestedById, requestedByTag)),
      kind: 'playlist',
      label: info.title?.trim() || (mix ? 'YouTube Mix' : 'the playlist'),
    };
  }
...
export function createAudioStream(opts: StreamOptions): AudioStream {
  ...
  const ytInput = new PassThrough({ highWaterMark: 1 << 20 });
  ytInput.pipe(ff.stdin);
  ff.stdin.on('error', () => {});
  ...
  let yt: ReturnType<typeof ytdlp.exec> | null = null;

  const startYtdlp = (): void => {
    yt = ytdlp.exec(
      opts.track!.url,
      { ...authFlags(), ...ytOptions },
      { stdio: ['ignore', 'pipe', 'pipe'] },
    ...
    const current = yt;
    if (!current.stdout) throw new Error('yt-dlp produced no audio stream.');
    let ytErrTail = '';
    current.stdout.on('error', () => {});
    current.stdout.pipe(ytInput, { end: false });
    current.stderr?.on('data', (d: Buffer) => {
      ytErrTail = (ytErrTail + d.toString()).slice(-600);
    });
    current.on?.('exit', (code: number | null, signal: string | null) => {
      if (signal === 'SIGKILL' || !code || code === 0) {
        if (!ytInput.destroyed) ytInput.end();
        return;
      }
    ...
    // youtube-dl-exec is both a ChildProcess and a rejecting promise. A
    // killed/failed child must never become an unhandled rejection.
    current.catch(() => {});
  };
